...
<p><a href="https://gitter.im/rmarquis/pacaur?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge"><img src="https://badges.gitter.im/rmarquis/pacaur.svg" alt="Join the chat at https://gitter.im/rmarquis/pacaur"></a></p>
...
binary and extend it with AUR functions B<(-S, -Ss, -Si, -Sw, -Su, -Sc, -Qu)>,
and commands which are AUR specific B<(-s, -i, -d, -m, -y, -u, -k)>. As such,
...
=item B<-S, -Ss, -Si, -Sw, -Su, -Sc, -Qu>

Pacaur wraps all B<pacman operations> and by default extends respectively its
install, search, info, download only, update, clean and check updates functions
...
Clone the I<target(s)> build files. Pass this option twice to fetch uninstalled
