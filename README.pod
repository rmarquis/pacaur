...
When used with the secure mode enabled, temporarily enable $CARCH bash magic
compatibility.
...
PKGBUILDs using the $CARCH makepkg variable might not be correctly handled
anymore. Default is B<false>.
