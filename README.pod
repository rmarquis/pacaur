...
Temporarily enable $CARCH bash magic compatibility when I<carchmagic> option is
disabled.
...
Enable editing of PKGBUILD when using pacman extended operations. Default is B<true>.

=item B<editinstall>

Enable editing of install script when using pacman extended operations. Default is B<true>.

=item B<cleandevel>

When clean config option is enabled, also clean build directory of development
packages. Default is B<true>.

=item B<carchmagic>

Enable compatibility with I<$CARCH> bash magic. When enabled, compatibility
with PKGBUILDs that use differents dependencies and makedependencies for
differents architectures in a single PKGBUILD through the $CARCH makepkg
variable is ensured, at the cost of dependencies solving speed.
Default is B<false>.
