...
info, download only, update, check updates and clean functions for the AUR. This
...
When searching the AUR, also show results marked as out of date. This takes
precedence over the I<IgnoreOOD> option in the cower config file.
...
Define the directory into which the package build files will be cloned.
Default is I<$AURDEST>, with a fallback on B</tmp/pacaurtmp-$USER>.
...
Display PKGBUILD and install script viewing prompt. Possible values are B<full>
to always display the full build files, B<diff> to only display the difference
since last build, and B<none> to disable the viewing prompt. Default is B<full>.
...
Determines where the package build files will be cloned.
...
Determines where the built packages will be stored.

=item B<SRCDEST>

Determines where the source of packages will be stored.

=item B<LOGDEST>

Determines where the build logs will be stored.

=item B<BUILDDIR>

Determines where the packages will be built.
...
To be able to display build files comparison, the default clone directory (set
to /tmp on a temporary filesystem residing in volatile memory) should be set
to a persistent location. Alternatively, define the pacaur specific I<$AURDEST>
...
The alternative installation root I<-r> short option of pacman is not supported.
You might want to use the I<--root /path> long option instead, or chroot to the 
new path and use pacaur from there instead.
