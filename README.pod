...
Edit I<target> PKGBUILD and view install script when downloading or building target. Can also be used
...
variables. These variables (such as $BUILDDIR) can be overriden in the config
file if needed. See B<cower>(1) and B<makepkg>(8).
...
Enable compatibility with I<$CARCH> bash magic. Ensure that architecture
independent PKGBUILDs using the $CARCH makepkg variable are correctly handled
by using a full bash dependency solver instead of cower's internal solver.
Default is B<true>.
