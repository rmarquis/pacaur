...
Define the build directory. Default is I<$AURDEST>, with a fallback on
...
When the B<clean> config option is enabled, this also cleans build directories
of development packages. Default is B<true>.
...
You might also want to change the default build directory which is
