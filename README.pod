...
manual build process with makepkg as well as being knownledgeable about sudo
configuration.
...
Perform commands without editing any installation files. Bypass I<--edit> option.

=item B<--rebuild>

Always rebuild package(s) regardless of any existing file in I<$PKGDEST> directory,
...
for such packages with the B<--insecure> option, while the additional B<--preview>
...
I<colored output> can be enabled with the B<Color> option, while a
I<detailed interface> displaying name, version and size of target packages
formatted as a table can be enabled with the B<VerbosePkgLists> option.
...
the makepkg I<$PKGDEST> environment variable is set or that the clean option is
disabled in order to avoid the packages to be deleted after being built.
...
the opinion that PKGBUILDs should always propose default compilation options.
