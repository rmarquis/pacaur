...
Enable PKGBUILD editing when using pacman extended operations.
Default is B<true>.

=item B<editinstall>

Enable install script editing when using pacman extended operations.
Default is B<true>.

=item B<cleandevel>

Clean build directory of development packages when clean config option is
enabled. Default is B<true>.
