...
To be able to display build files comparison, the I<displaybuildfiles> config
option should be set to I<diff>. The clone directory (set by default to
I<$XDG_CACHE_HOME/pacaur>) can be changed through the alternative
I<$AURDEST> environment variable. It should always be set to a persistent location.
...
are displayed while checking for updates although the internet connection is
...
B<[sk] - Slovenčina> - Pavol "pakapusta" Kapusta
