...
When searching, ignore all results marked as out of date.

=item B<--no-ignore-ood>

When searching, also show results marked as out of date. This take precedence
over the I<IgnoreOOD> option of the cower config file.

=item B<--insecure>

When using the secure mode, temporarily enable the full bash dependency solver
to allow full I<$CARCH> bash magic compatibility.
...
bash solver to enhance security. The PKGBUILDs using the B<$CARCH> makepkg
...
B<[it] - Italiano> - Demenico "NoMore201" Lezzi E<lt>https://github.com/NoMore201E<gt>
