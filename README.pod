...
When used with pacman extended operations, only search or install target(s) from
the AUR.
...
Do not reinstall I<target(s)> that are already up to date.
...
Always rebuild I<target(s)> regardless of any existing file in I<$PKGDEST> directory,
