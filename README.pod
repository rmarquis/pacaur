...
interface. It uses the full secured RPC interface to solve the dependency tree,
and it is built upon the well designed cower and expac C backends.
...
When used with install and update operations, also consider already installed
foreign dependencies packages.
