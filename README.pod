...
Enable silent output. The makepkg output will be collected in the clone
directory or the I<LOGDEST> directory if defined should any error occurs.
Default is B<false>.
...
files) will be cloned. You might want to change the default clone directory (set
to I</tmp> on a temporary filesystem residing in volatile memory) to make full
use of cloned build files.
...
located in volatile memory, unless big packages need to be built on a low-memory
machine.
...
To be able to display build files comparison, the default clone directory
(set to I</tmp> on a temporary filesystem residing in volatile memory) should
be set to a persistent location. Alternatively, define the pacaur specific
I<$AURDEST> environment variable.
